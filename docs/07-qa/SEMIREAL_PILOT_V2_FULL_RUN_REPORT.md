...
- Jobs total: 73
- Jobs by status: `{"failed": 1, "succeeded": 72}`
- Jobs by type: `{"classification": 20, "extraction": 33, "ingest": 20}`
- Job failures: 1

## Pre-Review Summary

- Counts: `{"chunks": 20, "facts": 49, "published_facts": 0, "published_rules": 0, "rules": 9, "sources": 20, "unknowns": 3}`
- Chunk statuses: `{"extracted": 18, "needs_review": 1, "pending": 1}`
- Fact statuses: `{"extracted": 49}`
- Rule statuses: `{"extracted": 9}`
- Unknown statuses: `{"open": 3}`
- Fact types: `{"business_hours": 31, "contact_info": 2, "faq_item": 4, "payment_method": 4, "service_price": 8}`
- Rule types: `{"cancellation_policy": 2, "discount_rule": 7}`

## Review/Publish Outputs

- Actions attempted: 58
- Failed actions: 0
- Post-review counts: `{"chunks": 20, "facts": 49, "published_facts": 49, "published_rules": 9, "rules": 9, "sources": 20, "unknowns": 3}`
- Post-review fact statuses: `{"published": 49}`
